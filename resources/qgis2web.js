...
var map = new ol.Map({
    ...
    view: new ol.View({
         maxZoom: 28, minZoom: 1, projection: new ol.proj.Projection({
            code: 'EPSG:3857',
            //extent: [-20037508.342789, -44927335.427097, 20037508.342789, 44927335.427097],
            units: 'm'})
    })
});

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([-8949743.024563, -4224854.305226, -2399337.519292, 1391062.623161], map.getSize());
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    var popupText = ''; // Initialize popupText
    var fieldLabels = layer.get('fieldLabels') || {}; // Get fieldLabels or empty object
    var fieldAliases = layer.get('fieldAliases') || {}; // Get fieldAliases or empty object
    var fieldImages = layer.get('fieldImages') || {}; // Get fieldImages or empty object

    // Iterate through the keys defined in fieldLabels first, to respect the desired order and filtering
    for (var key in fieldLabels) {
        // Skip geometry, internal properties, and fields explicitly marked as hidden
        if (key === 'geometry' || key === 'layerObject' || key === 'idO' || fieldLabels[key] === 'hidden field') {
            continue;
        }

        // Check if the feature actually has this key
        if (currentFeatureKeys.includes(key)) {
            var fieldValue = currentFeature.get(key);
            var fieldAlias = fieldAliases[key] || key; // Use alias if available, otherwise key name
            var labelType = fieldLabels[key];
            var imageType = fieldImages[key]; // Get image type
            var popupField = ''; // Initialize popupField for this iteration

            // Skip fields marked for inline display if they have no data
            if (labelType === "inline label - visible with data" && (fieldValue === null || String(fieldValue).trim() === '')) {
                continue;
            }
            // Skip fields marked for header display if they have no data
            if (labelType === "header label - visible with data" && (fieldValue === null || String(fieldValue).trim() === '')) {
                continue;
            }

            // Determine formatting based on labelType
            if (labelType === "inline label - always visible" || labelType === "inline label - visible with data") {
                popupField += '<th>' + fieldAlias + '</th><td>';
            } else if (labelType === "header label - always visible" || labelType === "header label - visible with data") {
                // Header label: Display alias as bold header, then value below
                popupField += '<td colspan="2"><strong>' + fieldAlias + '</strong><br />';
            } else {
                // Default case or unrecognized label type (maybe just show value?)
                // Let's default to showing the value without a specific label for now, assuming it should be visible
                 popupField += '<td colspan="2">';
            }

            // Format the value (handle links, images, videos, etc.)
            var formattedValue = (fieldValue !== null && fieldValue !== undefined) ? String(fieldValue).toLocaleString() : '';
            if (imageType && imageType === "ExternalResource") {
                if (/\.(gif|jpg|jpeg|tif|tiff|png|avif|webp|svg)$/i.test(formattedValue)) {
                    popupField += '<img src="images/' + formattedValue.replace(/[\\\/:]/g, '_').trim() + '" />';
                } else if (/\.(mp4|webm|ogg|avi|mov|flv)$/i.test(formattedValue)) {
                    popupField += '<video controls><source src="images/' + formattedValue.replace(/[\\\/:]/g, '_').trim() + '" type="video/mp4">Seu navegador não suporta a tag de vídeo.</video>';
                } else {
                    popupField += autolinker.link(formattedValue);
                }
            } else {
                // Default text formatting with autolinker
                popupField += autolinker.link(formattedValue);
            }
            
            popupField += '</td>'; // Close the td tag
            popupText += '<tr>' + popupField + '</tr>';
        }
    }
    return popupText; // Correct return statement
}
...
function onSingleClickFeatures(evt) {
    ...
    var clusteredFeatures;
    var popupText = '<ul>';
    var processedFeatures = new Set(); // Track processed features in this click event
    
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        if (layer && feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") === undefined)) {
            // Generate a unique key for the feature to avoid processing duplicates
            // Using layer title + feature id (if available) or properties as fallback
            var featureId = feature.getId ? feature.getId() : JSON.stringify(feature.getProperties());
            var featureKey = layer.get('popuplayertitle') + '_' + featureId;

            if (processedFeatures.has(featureKey)) {
                console.log('Skipping already processed feature:', featureKey); // Debugging log
                return; // Skip if already processed in this click event
            }
            processedFeatures.add(featureKey);

            var doPopup = false;
            // Check if *any* field is meant to be shown (not hidden)
            var fieldImages = layer.get('fieldImages') || {};
            for (var k in fieldImages) {
                if (fieldImages[k] !== "Hidden") {
                    doPopup = true;
                    break; // Found one visible field, no need to check further
                }
            }
            // Also check fieldLabels for visibility
            if (!doPopup) {
                var fieldLabelsCheck = layer.get('fieldLabels') || {};
                 for (var k in fieldLabelsCheck) {
                     if (fieldLabelsCheck[k] !== 'hidden field') {
                         doPopup = true;
                         break;
                     }
                 }
            ...
            if (typeof clusteredFeatures !== "undefined") {
                // Handle Clusters
                if (doPopup) {
                    // Add a header for the cluster itself if needed, or process individuals
                    popupText += '<li><table>';
                    popupText += '<caption>Cluster (' + clusteredFeatures.length + ' feições) - ' + layer.get('popuplayertitle') + '</caption>';
                    // Loop through features in the cluster
                    for(var n = 0; n < clusteredFeatures.length; n++) {
                        var clusterItemFeature = clusteredFeatures[n];
                        var clusterItemKeys = clusterItemFeature.getKeys();
                        // Optional: Add sub-header per item or just list fields
                        // We might want a simplified view for clusters, e.g., only showing the primary field.
                        // For now, let's call createPopupField but maybe simplify its output later if needed.
                        popupText += createPopupField(clusterItemFeature, clusterItemKeys, layer); 
                        if (n < clusteredFeatures.length - 1) popupText += '<tr><td colspan="2"><hr></td></tr>'; // Separator
                    }
                    popupText += '</table></li>';
                }
            } else {
                ...
                if (doPopup) {
                    popupText += '<li><table>';
                    // Use caption for title for better semantics
                    popupText += '<caption>' + layer.get('popuplayertitle') + '</caption>'; 
                    popupText += createPopupField(currentFeature, currentFeatureKeys, layer);
                    popupText += '</table></li>'; // Close table and list item here
                }
            }
        }
    });
